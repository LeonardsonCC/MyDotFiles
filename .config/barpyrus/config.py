...
conky_text += "${endif}"
conky_text += "%{F-}"
conky_text += '%{T-}%{F\\#989898} ${exec awk -F"[][]" \'/Left:/ { print $2 }\' <(amixer sget Master)} '

# example options for the hlwm.HLWMLayoutSwitcher widget
xkblayouts = [
    'us us -variant intl'.split(' '),
    'br br -variant abnt2'.split(' '),
]
...
grey_frame = Theme(bg = '#303030', fg = '#EFEFEF', padding = (5,5))

# Widget configuration:
bar = lemonbar.Lemonbar(geometry = (x,y,width,height), font=font)
